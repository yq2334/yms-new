		...
		<view>
			<view class="notice-content">
				<view class="wrapper">
					...
					<view class="detail margin-top" v-html="detail.body">
						<!-- 	<p> 当消费者进店或即将进店时，店员可以适当的进行引导，在消费者进店后与消费者及时沟通，适时的温馨提醒，可能消费者就会在店员的引导中，选择自己想喝的饮品。同时，促销也是通过感情拉动的，通过有效的引导和沟通，让消费者想要购买的欲望增强。
					</p>
					<p>
						       能留住顾客试喝，是获取潜在顾客进一步沟通的机会。顾客驻足、停留品尝试喝饮品，已经表示顾客接受了你的服务。
					</p> -->
					</view>
				</view>
		...
		methods: {
			getDetail() {
				getNewListDetail({
					...
					apiname: 'yhh.newsdetail',
					token: getToken(),
					Id: this.id
				}).then((res) => {
					console.log(res)
					this.detail = res.tzdetail
				})
			}
		},
	}
...
<style lang="scss" scoped>
	...
	.notice {
		&-content {
			...
			.wrapper {
				background: #FFFFFF;
				box-shadow: inset 0rpx 45rpx 110rpx 0rpx rgba(250, 197, 98, 0.35);
				border-radius: 17rpx;
				padding: 34rpx;
				...
				.msg {
					width: 47rpx;
					height: 47rpx;
					background: linear-gradient(-10deg, rgba(255, 153, 95, 0.9), rgba(255, 115, 36, 0.9));
