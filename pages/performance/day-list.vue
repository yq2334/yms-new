			...
			<view class="content">
				...
				<view class="list margin-top">
					<view @click="navToDetail(item)" class="item flex align-center justify-between margin-bottom-sm"
						...
						<view class="">
							<text class="margin-right-sm">{{item.riqi}}</text>
...
<script>
	...
	export default {
			...
			return {
				...
				paramsMap: {
					...
					jiaoyi_month_xiaji: {
						key: 'jiaoyi',
						team: {
							api: '/api_yejichaxun_jiaoyi_day_xiaji.ashx',
							name: 'yhh.yejichaxun_jiaoyi_day_xiaji'
						},
						direct: {
							api: '/api_yejichaxun_jiaoyi_day.ashx',
							name: 'yhh.yejichaxun_jiaoyi_day'
						}
					...
					jihuo_month_xiaji: {
						key: 'jihuo',
						team: {
							api: '/api_yejichaxun_jihuo_day_xiaji.ashx',
							name: 'yhh.yejichaxun_jihuo_day_xiaji'
						},
						direct: {
							api: '/api_yejichaxun_jihuo_day.ashx',
							name: 'yhh.yejichaxun_jihuo_day'
						}
		...
		onLoad(params) {
			...
			this.tabs = [{
					name: `本级${this.type}日报`,
					val: 0
				},
				{
					name: `团队${this.type}日报`,
					val: 1
				}
			];
		...
		methods: {
			...
			navToDetail(item) {
				...
				if (this.current == 0) {
					this.$tab.navigateTo('/pages/performance/direct-detail?name=' + 'day_' + this.paramsMap[this
							.params_name].key + '&type=day' + '&tab=' + this.paramsMap[this.params_name].key +
						'&riqi=' + item.riqi)

				} else {
					this.$tab.navigateTo('/pages/performance/team-detail?name=' + 'day_' + this.paramsMap[this.params_name]
						.key + '&type=day' +
						'&tab=' + this.paramsMap[this.params_name].key + '&riqi=' + item.riqi)
				}
			}
		},
	}
